...
//Unit: Gy
TETDRFScorer::TETDRFScorer(G4String name,TETModelImport* _PhantomData)
...
TETDRFScorer::~TETDRFScorer()
{;}

G4int TETDRFScorer::GetIndex(G4Step* aStep)
{
	G4int copyNo = aStep->GetPreStepPoint()->GetTouchable()->GetCopyNumber();
	return PhantomData->GetMaterialIndex(copyNo);
}

G4bool TETDRFScorer::ProcessHits(G4Step* aStep, G4TouchableHistory*)
{
...
void TETDRFScorer::Initialize(G4HCofThisEvent* HCE)
{
...
void TETDRFScorer::EndOfEvent(G4HCofThisEvent*)
{;}

void TETDRFScorer::clear()
{
	EvtMap->clear();
}

G4double TETDRFScorer::GetRBMdose(G4double energy, G4double cellFlux, G4int organID)
{
...
G4double TETDRFScorer::GetBSdose(G4double energy, G4double cellFlux, G4int organID){
...
G4int TETDRFScorer::FindIndexfromEnergyBin(G4double energy){
